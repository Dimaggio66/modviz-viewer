...
import { Upload, AlertTriangle, ChevronDown, ExternalLink, Plus, Clock3, PackagePlus, Cloud } from 'lucide-react';
      ...
      >
        ...
        <div className="absolute inset-0 z-10 overflow-auto p-4 md:p-8">
          ...
          <div {...tourAnchor(TOUR_ANCHORS.emptyStateCard)} className="max-w-md w-full rounded-2xl border border-zinc-300 bg-white p-8 shadow-xl transition-transform duration-200 hover:-translate-y-1 dark:border-[#3b4261] dark:bg-[#16161e] flex flex-col items-center">
            ...
            <div
              className="mb-8 w-full rounded-xl border border-slate-200 p-6 shadow-sm dark:border-slate-700"
              style={{ backgroundColor: '#f8fafc' }}
            >
              <img
                src="/modviz_logo_3d_centered.svg"
                alt="ModViz"
                className="mx-auto h-36 w-full max-w-sm object-contain sm:h-40"
              />
            </div>

            {/* Primary open / drag action. */}
            <button
              onClick={() => { void handleOpenClick(); }}
              disabled={!webgpu.supported || webgpu.checking}
              className={`group flex w-full items-center justify-center gap-3 rounded-xl border px-6 py-3 font-mono text-sm transition-all ${
            ...
            {/* Secondary actions for starting without a local IFC file. */}
            <div className="flex flex-wrap items-center justify-center gap-2">
              <button
                type="button"
                onClick={() => { void handleStartBlank(); }}
                disabled={!webgpu.supported || webgpu.checking}
                className={`group inline-flex items-center gap-1.5 rounded-lg border border-dashed px-3 py-1.5 font-mono text-[11px] transition-all ${
                  !webgpu.supported || webgpu.checking
                    ? 'border-zinc-200 dark:border-[#3b4261]/50 text-zinc-300 dark:text-[#565f89]/50 cursor-not-allowed'
